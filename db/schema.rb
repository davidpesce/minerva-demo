...
ActiveRecord::Schema.define(version: 20180827073946) do
  ...
  create_table "alignments", id: :serial, force: :cascade do |t|
    ...
    t.index ["resource_id", "taxonomy_id"], name: "index_alignments_on_resource_id_and_taxonomy_id", unique: true
    t.index ["status"], name: "index_alignments_status"
    t.index ["taxonomy_id", "resource_id"], name: "index_alignments_on_taxonomy_id_and_resource_id"
  end
  ...
  create_table "resource_stats", id: :serial, force: :cascade do |t|
    ...
    t.index ["resource_id", "taxonomy_id"], name: "index_resource_stats_on_resource_id_and_taxonomy_id", unique: true
  end
  ...
  create_table "resources_subjects", id: :serial, force: :cascade do |t|
    ...
    t.index ["resource_id", "subject_id"], name: "index_res_subj_on_res_id_and_subj_id"
    t.index ["subject_id", "resource_id"], name: "index_subj_res_on_res_id_and_subj_id", unique: true
  end

  create_table "sub_resources", id: :serial, force: :cascade do |t|
    t.integer "parent_id"
    t.integer "child_id"
    t.index ["child_id", "parent_id"], name: "index_sub_resources_on_child_id_and_parent_id"
    t.index ["parent_id", "child_id"], name: "index_sub_resources_on_parent_id_and_child_id", unique: true
  end
...
$function$
  ...
  # no candidate create_trigger statement could be found, creating an adapter-specific one
  execute(<<-TRIGGERSQL)
CREATE OR REPLACE FUNCTION public.resources_after_update_of_name_description_row_tr()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
...
$function$
  ...
  # no candidate create_trigger statement could be found, creating an adapter-specific one
  execute("CREATE TRIGGER resources_after_update_of_name_description_row_tr AFTER UPDATE OF name, description ON resources FOR EACH ROW EXECUTE PROCEDURE resources_after_update_of_name_description_row_tr()")
